...
arch -x86_64 pod repo push Chaos "${podspec_name}" --allow-warnings --verbose --sources=https://github.com/CocoaPods/Specs.git,ssh://git@code.kh-super.net:7999/mob/chaos-specs.git
echo "发布 $filename 版本 $version 到 Chaos 结束\n"

echo "开始打包 framework"
arch -x86_64 pod package ${podspec_name} --no-mangle --exclude-deps --force --spec-sources=https://github.com/CocoaPods/Specs.git,ssh://git@code.kh-super.net:7999/mob/chaos-specs.git

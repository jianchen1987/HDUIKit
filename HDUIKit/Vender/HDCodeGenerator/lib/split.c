...
static int Split_eatNum(const char *string, QRinput *input, QRencodeMode hint) {
    ...
    if (mode == QR_MODE_8) {
        dif = QRinput_estimateBitsModeNum(run) + 4 + ln + QRinput_estimateBitsMode8(1) /* + 4 + l8 */
    ...
    if (mode == QR_MODE_AN) {
        dif = QRinput_estimateBitsModeNum(run) + 4 + ln + QRinput_estimateBitsModeAn(1) /* + 4 + la */
...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint) {
    ...
    while (isalnum(*p)) {
        if (isdigit(*p)) {
            ...
            dif = QRinput_estimateBitsModeAn((int)(p - string)) /* + 4 + la */
                + QRinput_estimateBitsModeNum((int)(q - p)) + 4 + ln - QRinput_estimateBitsModeAn((int)(q - string)) /* - 4 - la */;
            if (dif < 0) {
                break;
            } else {
                p = q;
    ...
    if (*p && !isalnum(*p)) {
        dif = QRinput_estimateBitsModeAn(run) + 4 + la + QRinput_estimateBitsMode8(1) /* + 4 + l8 */
            - QRinput_estimateBitsMode8(run + 1) /* - 4 - l8 */;
        if (dif > 0) {
            return Split_eat8(string, input, hint);
        }
    ...
    while (*p != '\0') {
        ...
        if (mode == QR_MODE_NUM) {
            ...
            dif = QRinput_estimateBitsMode8((int)(p - string)) /* + 4 + l8 */
                + QRinput_estimateBitsModeNum((int)(q - p)) + 4 + ln - QRinput_estimateBitsMode8((int)(q - string)) /* - 4 - l8 */;
            if (dif < 0) {
                break;
            } else {
                p = q;
            ...
            dif = QRinput_estimateBitsMode8((int)(p - string)) /* + 4 + l8 */
                + QRinput_estimateBitsModeAn((int)(q - p)) + 4 + la - QRinput_estimateBitsMode8((int)(q - string)) /* - 4 - l8 */;
            if (dif < 0) {
                break;
            } else {
                p = q;

...
@interface HDCategoryListContainerViewController : UIViewController
...
@implementation HDCategoryListContainerViewController
- (void)dealloc {
...
@interface HDCategoryListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<HDCategoryListContainerViewDelegate> delegate;
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, id<HDCategoryListContentViewDelegate>> *validListDict;
@property (nonatomic, assign) NSInteger willAppearIndex;
@property (nonatomic, assign) NSInteger willDisappearIndex;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) HDCategoryListContainerViewController *containerVC;
@end

@implementation HDCategoryListContainerView

- (instancetype)initWithType:(HDCategoryListContainerType)type delegate:(id<HDCategoryListContainerViewDelegate>)delegate {
...
- (void)initializeViews {
    ...
    _containerVC = [[HDCategoryListContainerViewController alloc] init];
    ...
    if (self.containerType == HDCategoryListContainerTypeScrollView) {
        if (self.delegate &&
...
- (void)layoutSubviews {
    ...
    if (self.containerType == HDCategoryListContainerTypeScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) || !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull index, id<HDCategoryListContentViewDelegate> _Nonnull list, BOOL *_Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<HDCategoryListContentViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
#pragma mark - HDCategoryViewListContainer
...
- (void)reloadData {
    for (id<HDCategoryListContentViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<HDCategoryListContentViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        // 列表已经创建好了
        return;
    }

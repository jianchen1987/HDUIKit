...
@interface HDCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) HDCategoryCollectionView *collectionView;
...
@implementation HDCategoryBaseView
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    HDCategoryBaseCellModel *cellModel = self.dataSource[index];
    cellModel.selectedType = HDCategoryCellSelectedTypeUnknown;
    [self refreshCellModel:cellModel index:index];
    HDCategoryBaseCell *cell = (HDCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    [cell reloadData:cellModel];
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:HDCategoryCellSelectedTypeCode];
}

- (void)layoutSubviews {
    [super layoutSubviews];

    //部分使用者为了适配不同的手机屏幕尺寸，HDCategoryView的宽高比要求保持一样，所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
    //如果向下取整导致了你的页面异常，请自己重新设置HDCategoryView的高度，保证为整数即可。
...
- (void)setDelegate:(id<HDCategoryViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)setListContainer:(id<HDCategoryViewListContainer>)listContainer {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    HDCategoryBaseCellModel *cellModel = self.dataSource[indexPath.item];
    cellModel.selectedType = HDCategoryCellSelectedTypeUnknown;
    [(HDCategoryBaseCell *)cell reloadData:cellModel];
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL isTransitionAnimating = NO;
    for (HDCategoryBaseCellModel *cellModel in self.dataSource) {
...
- (CGFloat)getContentEdgeInsetLeft {
    if (self.contentEdgeInsetLeft == HDCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (CGFloat)getContentEdgeInsetRight {
    if (self.contentEdgeInsetRight == HDCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (void)clickSelectItemAtIndex:(NSInteger)index {
    ...
    [self selectCellAtIndex:index selectedType:HDCategoryCellSelectedTypeClick];
}

- (void)scrollSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:HDCategoryCellSelectedTypeScroll];
}
...
@implementation HDCategoryBaseView (UISubclassingBaseHooks)

- (CGRect)getTargetCellFrame:(NSInteger)targetIndex {
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i++) {
        HDCategoryBaseCellModel *cellModel = self.dataSource[i];
        CGFloat cellWidth;
    ...
    CGFloat width;
    HDCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
...
- (CGRect)getTargetSelectedCellFrame:(NSInteger)targetIndex selectedType:(HDCategoryCellSelectedType)selectedType {
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i++) {
        HDCategoryBaseCellModel *cellModel = self.dataSource[i];
...
- (void)initializeData {
    ...
    _selectedIndex = 0;
    _cellWidth = HDCategoryViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = HDCategoryViewAutomaticDimension;
    _contentEdgeInsetRight = HDCategoryViewAutomaticDimension;
...
- (void)initializeViews {
    ...
    _collectionView = [[HDCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)refreshState {
    ...
    if (self.isAverageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(HDCategoryBaseCellModel *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            obj.cellSpacing = self.innerCellSpacing;
        }];
    }

    // ---------------------定位 collectionView 到当前选中的位置----------------------
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(HDCategoryBaseCellModel *cellModel, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况系统会出现HDCategoryView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
    ...
    // 将contentScrollView的 contentOffset 定位到当前选中index的位置
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(HDCategoryCellSelectedType)selectedType {
    ...
    if (self.selectedIndex == targetIndex) {
        ...
        if (selectedType == HDCategoryCellSelectedTypeCode) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        } else if (selectedType == HDCategoryCellSelectedTypeClick) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        ...
        } else if (selectedType == HDCategoryCellSelectedTypeScroll) {
    ...
    // 通知子类刷新当前选中的和将要选中的 cellModel
    HDCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    ...
    // 刷新当前选中的和将要选中的 cell
    HDCategoryBaseCell *lastCell = (HDCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    HDCategoryBaseCell *selectedCell = (HDCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    [selectedCell reloadData:selectedCellModel];

    if (self.scrollingTargetIndex != -1 && self.scrollingTargetIndex != targetIndex) {
        HDCategoryBaseCellModel *scrollingTargetCellModel = self.dataSource[self.scrollingTargetIndex];
        ...
        HDCategoryBaseCell *scrollingTargetCell = (HDCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.scrollingTargetIndex inSection:0]];
    ...
    if (selectedType == HDCategoryCellSelectedTypeClick ||
        selectedType == HDCategoryCellSelectedTypeCode) {
...
- (void)refreshSelectedCellModel:(HDCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(HDCategoryBaseCellModel *)unselectedCellModel {
    ...
    if (self.isCellWidthZoomEnabled) {
        if (selectedCellModel.selectedType == HDCategoryCellSelectedTypeCode ||
            selectedCellModel.selectedType == HDCategoryCellSelectedTypeClick) {
            ...
            self.animator.progressCallback = ^(CGFloat percent) {
                ...
                selectedCellModel.cellWidthCurrentZoomScale = [HDCategoryFactory interpolationFrom:selectedCellModel.cellWidthNormalZoomScale to:selectedCellModel.cellWidthSelectedZoomScale percent:percent];
                ...
                unselectedCellModel.cellWidthCurrentZoomScale = [HDCategoryFactory interpolationFrom:unselectedCellModel.cellWidthSelectedZoomScale to:unselectedCellModel.cellWidthNormalZoomScale percent:percent];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.animator.isExecuting) {
            ...
            for (HDCategoryBaseCellModel *model in self.dataSource) {
                if (model.isSelected) {
        ...
        if (self.isCellWidthZoomEnabled && self.isCellWidthZoomScrollGradientEnabled) {
            HDCategoryBaseCellModel *leftCellModel = (HDCategoryBaseCellModel *)self.dataSource[baseIndex];
            HDCategoryBaseCellModel *rightCellModel = (HDCategoryBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthCurrentZoomScale = [HDCategoryFactory interpolationFrom:leftCellModel.cellWidthSelectedZoomScale to:leftCellModel.cellWidthNormalZoomScale percent:remainderRatio];
            leftCellModel.cellWidth = [self getCellWidthAtIndex:leftCellModel.index] * leftCellModel.cellWidthCurrentZoomScale;
            rightCellModel.cellWidthCurrentZoomScale = [HDCategoryFactory interpolationFrom:rightCellModel.cellWidthNormalZoomScale to:rightCellModel.cellWidthSelectedZoomScale percent:remainderRatio];
...
- (Class)preferredCellClass {
    return HDCategoryBaseCell.class;
}

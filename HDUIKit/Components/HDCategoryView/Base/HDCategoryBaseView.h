...
@class HDCategoryBaseView;

@protocol HDCategoryViewListContainer <NSObject>
...
/**
 ...
 @param index 选中的index
 */
- (void)categoryView:(HDCategoryBaseView *)categoryView didScrollSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @return 能否点击
 */
- (BOOL)categoryView:(HDCategoryBaseView *)categoryView canClickItemAtIndex:(NSInteger)index;
 ...
 */
- (void)categoryView:(HDCategoryBaseView *)categoryView scrollingFromLeftIndex:(NSInteger)leftIndex toRightIndex:(NSInteger)rightIndex ratio:(CGFloat)ratio;

@end

@interface HDCategoryBaseView : UIView

@property (nonatomic, strong, readonly) HDCategoryCollectionView *collectionView;
/// 数据源
@property (nonatomic, copy) NSArray<HDCategoryBaseCellModel *> *dataSource;
/// 代理
@property (nonatomic, weak) id<HDCategoryViewDelegate> delegate;
/**
 高封装度的列表容器，使用该类可以让列表拥有完成的生命周期、自动同步defaultSelectedIndex、自动调用reloadData。
 */
@property (nonatomic, weak) id<HDCategoryViewListContainer> listContainer;
/// 只用于获取
...
/// 整体内容的左边距，默认HDCategoryViewAutomaticDimension（等于cellSpacing）
@property (nonatomic, assign) CGFloat contentEdgeInsetLeft;
/// 整体内容的右边距，默认HDCategoryViewAutomaticDimension（等于cellSpacing）
@property (nonatomic, assign) CGFloat contentEdgeInsetRight;
/// 默认HDCategoryViewAutomaticDimension
@property (nonatomic, assign) CGFloat cellWidth;
/// cell宽度补偿。默认：0
@property (nonatomic, assign) CGFloat cellWidthIncrement;
/// cell之间的间距，默认20
@property (nonatomic, assign) CGFloat cellSpacing;
/// 当collectionView.contentSize.width小于HDCategoryBaseView的宽度，是否将cellSpacing均分。默认为 YES
...
/**
 刷新指定的index的cell
 内部会触发`- (void)refreshCellModel:(HDCategoryBaseCellModel *)cellModel index:(NSInteger)index`方法进行cellModel刷新
...
@interface HDCategoryBaseView (UISubclassingBaseHooks)
 ...
 */
- (void)refreshSelectedCellModel:(HDCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(HDCategoryBaseCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
...
/**
 ...
 @param index 选中的index
 @param selectedType HDCategoryCellSelectedType
 @return 返回值为NO，表示触发内部某些判断（点击了同一个cell），子类无需后续操作。
 */
- (BOOL)selectCellAtIndex:(NSInteger)index selectedType:(HDCategoryCellSelectedType)selectedType NS_REQUIRES_SUPER;
 ...
 */
- (void)refreshCellModel:(HDCategoryBaseCellModel *)cellModel index:(NSInteger)index NS_REQUIRES_SUPER;

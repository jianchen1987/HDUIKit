...
- (instancetype)initWithInputUnitCount:(NSUInteger)count {
    return [self initWithStyle:HDUnitTextFieldStyleBorder inputUnitCount:count];
}

- (instancetype)initWithStyle:(HDUnitTextFieldStyle)style inputUnitCount:(NSUInteger)count {
    if (self = [super initWithFrame:CGRectZero]) {
        NSCAssert(count > 0, @"HDUnitTextField must have one or more input units.");
        NSCAssert(count <= 8, @"HDUnitTextField can not have more than 8 input units.");
...
- (void)initialize {
    ...
    UITextRange *aNewRange = [HDUnitTextFieldTextRange rangeWithStart:point end:point];
...
- (void)_fillRect:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    CGFloat radius = _style == HDUnitTextFieldStyleBorder ? _borderRadius : 0;
...
- (void)_drawBorder:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    if (_style == HDUnitTextFieldStyleBorder) {
...
- (void)_drawText:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    for (int i = 0; i < _characterArray.count; i++) {
        ...
        CGFloat yOffset = _style == HDUnitTextFieldStyleBorder ? 0 : _borderWidth;
...
- (void)_resetCursorStateIfNeeded {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGFloat yOffset = self->_style == HDUnitTextFieldStyleBorder ? 0 : self->_borderWidth;
...
- (void)insertText:(NSString *)text {
    ...
    if ([self.delegate respondsToSelector:@selector(unitTextField:shouldChangeCharactersInRange:replacementString:)]) {
        if ([self.delegate unitTextField:self shouldChangeCharactersInRange:NSMakeRange(self.text.length, text.length) replacementString:text] == NO) {
            return;
        }
    }
...
/**
 支持 iOS12 短信验证码, keyboardType must be UIKeyboardTypeNumberPad to localizable.
...
- (nullable NSString *)textInRange:(HDUnitTextFieldTextRange *)range {
    return self.text;
}

- (void)replaceRange:(HDUnitTextFieldTextRange *)range withText:(NSString *)text {
}
...
- (UITextPosition *)beginningOfDocument {
    return [HDUnitTextFieldTextPosition positionWithOffset:0];
}
...
- (UITextPosition *)endOfDocument {
    ...
    return [HDUnitTextFieldTextPosition positionWithOffset:self.text.length - 1];
}
...
- (nullable UITextRange *)textRangeFromPosition:(HDUnitTextFieldTextPosition *)fromPosition toPosition:(HDUnitTextFieldTextPosition *)toPosition {
    NSRange range = NSMakeRange(MIN(fromPosition.offset, toPosition.offset), ABS(toPosition.offset - fromPosition.offset));
    return [HDUnitTextFieldTextRange rangeWithRange:range];
}

- (nullable UITextPosition *)positionFromPosition:(HDUnitTextFieldTextPosition *)position offset:(NSInteger)offset {
    NSInteger end = position.offset + offset;
    if (end > self.text.length || end < 0)
        return nil;
    return [HDUnitTextFieldTextPosition positionWithOffset:end];
}

- (nullable UITextPosition *)positionFromPosition:(HDUnitTextFieldTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    return [HDUnitTextFieldTextPosition positionWithOffset:position.offset + offset];
}

/* Simple evaluation of positions */
- (NSComparisonResult)comparePosition:(HDUnitTextFieldTextPosition *)position toPosition:(HDUnitTextFieldTextPosition *)other {
    if (position.offset < other.offset) return NSOrderedAscending;
    if (position.offset > other.offset) return NSOrderedDescending;
    return NSOrderedSame;
}

- (NSInteger)offsetFromPosition:(HDUnitTextFieldTextPosition *)from toPosition:(HDUnitTextFieldTextPosition *)toPosition {

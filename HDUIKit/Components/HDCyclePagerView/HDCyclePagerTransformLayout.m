...
typedef NS_ENUM(NSUInteger, HDTransformLayoutItemDirection) {
    HDTransformLayoutItemLeft,
    HDTransformLayoutItemCenter,
    HDTransformLayoutItemRight,
};

@interface HDCyclePagerTransformLayout () {
    struct {
...
@interface HDCyclePagerViewLayout ()

@property (nonatomic, weak) UIView *pageView;

@end

@implementation HDCyclePagerTransformLayout
...
- (void)setDelegate:(id<HDCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(HDCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (HDTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    HDTransformLayoutItemDirection direction = HDTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = HDTransformLayoutItemCenter;
    } else if (centerX - contentCenterX < 0) {
        direction = HDTransformLayoutItemLeft;
    }
    return direction;
}
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    return _layout.layoutType == HDCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != HDCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if (_layout.layoutType != HDCyclePagerTransformLayoutNormal) {
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(HDCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case HDCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case HDCyclePagerTransformLayoutCoverflow: {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(HDCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case HDCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        HDTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case HDTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width * (1 - scale) / 2;
                break;
            case HDTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width * (1 - scale) / 2;
                break;
            default:
                // center
...
@implementation HDCyclePagerViewLayout

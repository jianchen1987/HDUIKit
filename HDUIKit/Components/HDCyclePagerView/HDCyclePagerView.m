...
NS_INLINE BOOL TYEqualIndexSection(HDIndexSection indexSection1, HDIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE HDIndexSection TYMakeIndexSection(NSInteger index, NSInteger section) {
    HDIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface HDCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HDCyclePagerTransformLayoutDelegate> {
    struct {
...
@property (nonatomic, assign) HDIndexSection beginDragIndexSection;
...
@implementation HDCyclePagerView
...
- (void)addCollectionView {
    HDCyclePagerTransformLayout *layout = [[HDCyclePagerTransformLayout alloc] init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:HDPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (HDCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<HDCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((HDCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<HDCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(HDPagerScrollDirection)direction animate:(BOOL)animate {
    HDIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)updateLayout {
    ...
    ((HDCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(HDIndexSection)indexSection {
...
- (HDIndexSection)nearlyIndexPathAtDirection:(HDPagerScrollDirection)direction {
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (HDIndexSection)nearlyIndexPathForIndexSection:(HDIndexSection)indexSection direction:(HDPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }

    if (!_isInfiniteLoop) {
        if (direction == HDPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? TYMakeIndexSection(0, 0) : indexSection;
        } else if (direction == HDPagerScrollDirectionRight) {
    ...
    if (direction == HDPagerScrollDirectionRight) {
...
- (HDIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([_delegate respondsToSelector:@selector(pagerView:willDisplayCell:atIndexSection:)]) {
        [_delegate pagerView:self willDisplayCell:cell atIndexSection:TYMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    HDIndexSection newIndexSection = [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    HDIndexSection indexSection = _indexSection;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    HDPagerScrollDirection direction = HDPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = HDPagerScrollDirectionLeft;
    }
    HDIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - HDCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((HDCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;

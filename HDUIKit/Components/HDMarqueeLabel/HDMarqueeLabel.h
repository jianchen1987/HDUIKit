...
//  DongKe
//
//  Created by VanJay on 2018/9/28.
//  Copyright © 2018 DongKe. All rights reserved.
//

#import <UIKit/UIKit.h>

/** An enum that defines the types of `HDMarqueeLabel` scrolling */
typedef NS_ENUM(NSUInteger, HDMarqueeType) {
    /** Scrolls left first, then back right to the original position. */
    HDMarqueeTypeLeftRight = 0,
    /** Scrolls right first, then back left to the original position. */
    HDMarqueeTypeRightLeft = 1,
    ...
    HDMarqueeTypeContinuousReverse = 3
};
...
/**
 HDMarqueeLabel is a UILabel subclass adds a scrolling marquee effect when the text of a label instance outgrows the available width. Instances of `HDMarqueeLabel` can be configured
 for label scrolling direction/looping, speed/rate, and other options.
 */

IB_DESIGNABLE

@interface HDMarqueeLabel : UILabel

////////////////////////////////////////////////////////////////////////////////
/// @name Creating HDMarqueeLabels
////////////////////////////////////////////////////////////////////////////////

/** Returns a newly initialized `HDMarqueeLabel` instance.
 ...
 @return An initialized `HDMarqueeLabel` object or nil if the object couldn't be created.
 */

- (instancetype)initWithFrame:(CGRect)frame;

/** Returns a newly initialized `HDMarqueeLabel` instance with the specified scroll rate and edge transparency fade length.
 ...
 @param fadeLength A length of transparency fade at the left and right edges of the `HDMarqueeLabel` instance's frame.
 @see fadeLength
 @return An initialized `HDMarqueeLabel` object or nil if the object couldn't be created.
 */
...
/** Returns a newly initialized `HDMarqueeLabel` instance with the specified scroll duration and edge transparency fade length.
...
/** A boolean property that sets whether the `HDMarqueeLabel` should behave like a normal UILabel.
 
 When set to `YES` the `HDMarqueeLabel` will behave like a normal UILabel, and  will not begin scrolling when the text is
 larger than the specified frame. The change takes effect immediately, removing any in-flight animation as well as any
 current edge fade. Note that the `HDMarqueeLabel` will respect the current values of the `lineBreakMode` and `textAlignment`
...
/** A boolean property that sets whether the `HDMarqueeLabel` should hold (prevent) label scrolling.
 
 When set to `YES`, the `HDMarqueeLabel` will not automatically scroll even its text is larger than the specified frame,
 although the specified edge fades will remain.
 
 To set the `HDMarqueeLabel` to act like a normal UILabel, use the `labelize` property.
...
/** A boolean property that sets whether the `HDMarqueeLabel` should only begin a scroll when tapped.
 
 If this property is set to `YES`, the `HDMarqueeLabel` will begin a scroll animation cycle only when tapped. The label will
...
/** Defines the direction and method in which the `HDMarqueeLabel` instance scrolls.
 
 `HDMarqueeLabel` supports four types of scrolling: `MLLeftRight`, `MLRightLeft`, `MLContinuous`, and `MLContinuousReverse`.
...
/** The length of transparency fade at the left and right edges of the `HDMarqueeLabel` instance's frame.
 
 This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `HDMarqueeLabel`. The
...
/** Convenience method to restart all `HDMarqueeLabel` instances that have the specified view controller in their next responder chain.
 
 This method sends a `NSNotification` to all `HDMarqueeLabel` instances with the specified view controller in their next responder chain.
 ...
 @warning View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `HDMarqueeLabel` text
...
/** Labelizes all `HDMarqueeLabel` instances that have the specified view controller in their next responder chain.
 
 This method sends an `NSNotification` to all `HDMarqueeLabel` instances with the specified view controller in their next
 responder chain. The `labelize` property of these `HDMarqueeLabel` instances will be set to `YES`.
 
 @param controller The view controller for which all `HDMarqueeLabel` instances should be labelized.
 @see labelize
 */

+ (void)controllerLabelsShouldLabelize:(UIViewController *)controller;

/** De-Labelizes all `HDMarqueeLabel` instances that have the specified view controller in their next responder chain.
 
 This method sends an `NSNotification` to all `HDMarqueeLabel` instances with the specified view controller in their next
 responder chain. The `labelize` property of these `HDMarqueeLabel` instances will be set to `NO` .
 
 @param controller The view controller for which all `HDMarqueeLabel` instances should be de-labelized.
 @see labelize
 */

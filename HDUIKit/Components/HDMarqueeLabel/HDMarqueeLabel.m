...
NSString *const kHDMarqueeLabelShouldLabelizeNotification = @"HDMarqueeLabelShouldLabelizeNotification";
NSString *const kHDMarqueeLabelShouldAnimateNotification = @"HDMarqueeLabelShouldAnimateNotification";
NSString *const kHDMarqueeLabelAnimationCompletionBlock = @"HDMarqueeLabelAnimationCompletionBlock";
...
@interface UIView (HDMarqueeLabelHelpers)
- (UIViewController *)firstAvailableViewController;
- (id)traverseResponderChainForFirstViewController;
@end

@interface CAMediaTimingFunction (HDMarqueeLabelHelpers)
...
@implementation HDMarqueeLabel

#pragma mark - Class Methods and handlers

+ (void)restartLabelsOfController:(UIViewController *)controller {
    [HDMarqueeLabel notifyController:controller
                         withMessage:kHDMarqueeLabelControllerRestartNotification];
}

+ (void)controllerViewWillAppear:(UIViewController *)controller {
    [HDMarqueeLabel restartLabelsOfController:controller];
}
...
+ (void)controllerLabelsShouldLabelize:(UIViewController *)controller {
    [HDMarqueeLabel notifyController:controller
                         withMessage:kHDMarqueeLabelShouldLabelizeNotification];
}

+ (void)controllerLabelsShouldAnimate:(UIViewController *)controller {
    [HDMarqueeLabel notifyController:controller
                         withMessage:kHDMarqueeLabelShouldAnimateNotification];
}
...
- (void)forwardPropertiesToSubLabel {
    ...
    NSArray *properties = @[@"baselineAdjustment", @"enabled", @"highlighted", @"highlightedTextColor", @"minimumFontSize", @"textAlignment", @"userInteractionEnabled", @"adjustsFontSizeToFitWidth", @"lineBreakMode", @"numberOfLines"];
...
- (void)setupLabel {
    ...
    _marqueeType = HDMarqueeTypeContinuous;
    _awayOffset = 0.0f;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewControllerShouldRestart:) name:kHDMarqueeLabelControllerRestartNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldLabelize:) name:kHDMarqueeLabelShouldLabelizeNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldAnimate:) name:kHDMarqueeLabelShouldAnimateNotification object:nil];
...
#pragma mark - HDMarqueeLabel Heavy Lifting
...
- (void)updateSublabelAndBeginScroll:(BOOL)beginScroll {
    ...
    if (!self.labelShouldScroll) {
        ...
        switch (self.marqueeType) {
            case HDMarqueeTypeContinuousReverse:
            case HDMarqueeTypeRightLeft:
    ...
    switch (self.marqueeType) {
        case HDMarqueeTypeContinuous:
        case HDMarqueeTypeContinuousReverse: {
            if (self.marqueeType == HDMarqueeTypeContinuous) {
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval delayAmount:(NSTimeInterval)delayAmount {
    ...
    [awayAnim setValue:@(YES) forKey:kHDMarqueeLabelAnimationCompletionBlock];
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval
    ...
    [labelAnimation setValue:@(YES) forKey:kHDMarqueeLabelAnimationCompletionBlock];
...
- (void)setNumberOfLines:(NSInteger)numberOfLines {
    // By the nature of HDMarqueeLabel, this is 1
    [super setNumberOfLines:1];
}

- (void)setAdjustsFontSizeToFitWidth:(BOOL)adjustsFontSizeToFitWidth {
    // By the nature of HDMarqueeLabel, this is NO
...
- (void)setMarqueeType:(HDMarqueeType)marqueeType {
...
@implementation UIView (HDMarqueeLabelHelpers)
// Thanks to Phil M
...
@implementation CAMediaTimingFunction (HDMarqueeLabelHelpers)
...
- (CGFloat)solveTForY:(CGFloat)y_0 withEpsilon:(CGFloat)epsilon controlPoints:(NSArray *)controlPoints {
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (fabs(df0) < 1e-6) {
            NSLog(@"HDMarqueeLabel: Newton's Method failure, small/zero derivative!");
            break;
        }
        // Else recalculate t1
        t1 = t0 - f0 / df0;
    ...
    NSLog(@"HDMarqueeLabel: Failed to find t for Y input!");
    return t0;
}

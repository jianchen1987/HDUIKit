...
// 私有的类，专用于w实现 HDTextViewDelegate，避免 self.delegate = self 的写法（以前是 HDTextView 自己实现了 delegate）
@interface _HDTextViewDelegator : NSObject <HDTextViewDelegate>
...
@property (nonatomic, strong) _HDTextViewDelegator *delegator;
...
- (void)didInitialize {
    ...
    self.hd_multipleDelegatesEnabled = YES;
    self.delegator = [[_HDTextViewDelegator alloc] init];
...
- (void)handleTextChanged:(id)sender {
    ...
    HDTextView *textView = nil;

    if ([sender isKindOfClass:[NSNotification class]]) {
        id object = ((NSNotification *)sender).object;
        if (object == self) {
            textView = (HDTextView *)object;
        }
    } else if ([sender isKindOfClass:[HDTextView class]]) {
        textView = (HDTextView *)sender;
    }
    ...
    if (textView) {
        ...
        if ([textView.delegate respondsToSelector:@selector(textView:newHeightAfterTextChanged:)]) {
            ...
            if (textView.debug) HDLogInfo(NSStringFromClass(textView.class), @"handleTextDidChange, text = %@, resultHeight = %f", textView.text, resultHeight);
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [textView hd_scrollCaretVisibleAnimated:NO];
        });
    }
}
...
- (NSUInteger)lengthWithString:(NSString *)string {
    return self.shouldCountingNonASCIICharacterAsTwo ? string.hd_lengthWhenCountingNonASCIICharacterAsTwo : string.length;
}

- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    if (!self.shouldRejectSystemScroll) {
        [super setContentOffset:contentOffset animated:animated];
        if (self.debug) HDLogInfo(NSStringFromClass(self.class), @"%@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    } else {
        if (self.debug) HDLogInfo(NSStringFromClass(self.class), @"被屏蔽的 %@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    }
}
...
@implementation _HDTextViewDelegator

#pragma mark - <HDTextViewDelegate>

- (BOOL)textView:(HDTextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (self.textView.debug) HDLogInfo(NSStringFromClass(self.class), @"textView.text(%@ | %@) = %@\nmarkedTextRange = %@\nrange = %@\ntext = %@", @(textView.text.length), @(textView.text.hd_lengthWhenCountingNonASCIICharacterAsTwo), textView.text, textView.markedTextRange, NSStringFromRange(range), text);
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        NSUInteger rangeLength = textView.shouldCountingNonASCIICharacterAsTwo ? [textView.text substringWithRange:range].hd_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if (substringLength > 0 && [textView lengthWithString:text] > substringLength) {
                NSString *allowedText = [text hd_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
...
- (void)textViewDidChange:(HDTextView *)textView {
    ...
    if (!textView.markedTextRange) {
        if ([textView lengthWithString:textView.text] > textView.maximumTextLength) {
            ...
            textView.text = [textView.text hd_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textView.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
